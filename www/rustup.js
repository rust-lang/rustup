...
async function detect_platform() {
  ...
  if (
    ...
    (await navigator.userAgentData
      ...
      .then((ua) => ua.architecture == "arm" && ua.bitness == "64"))
  ) {
    os = "win-arm64";
  }
  ...
  if (navigator.oscpu) {
    if (navigator.oscpu.indexOf("Win32") != -1) {
      os = "win32";
    }
    if (navigator.oscpu.indexOf("Win64") != -1) {
      os = "win64";
    }
    ...
    if (navigator.oscpu.indexOf("SunOS") != -1) {
      os = "unix";
    }
  }
...
function process_copy_button_click(id) {
  try {
    navigator.clipboard
      .writeText(rustup_install_command)
      .then(() => (document.getElementById(id).style.opacity = "1"));
