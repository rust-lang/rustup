...
impl Configuration {
	pub fn new(prefix: InstallPrefix) -> Result<Option<Self>> {
		...
		if utils::is_file(&path) {
			...
			Ok(Some(Configuration { prefix: prefix, config: try!(Config::parse(&data)) }))
		} else {
			Ok(None)
		}
	}
	...
	pub fn init(prefix: InstallPrefix, config: Config) -> Result<Self> {
		...
		let data = try!(utils::read_file("config", &path));
		Ok(Configuration { prefix: prefix, config: try!(Config::parse(&data)) })
	}
	...
	pub fn update_dist_manifest(&self, url: &str, download: DownloadCfg) -> Result<()> {
		...
		try!(utils::rename_file("dist manifest", &new_dist, &self.prefix.manifest_file(manifest::DIST_MANIFEST)));
