...
impl<'a> ChangedItem<'a> {
	fn roll_back(&self, prefix: &InstallPrefix) -> Result<()> {
		...
		match *self {
			AddedFile(ref path) =>
				try!(utils::remove_file("component", &prefix.abs_path(path))),
			AddedDir(ref path) =>
				try!(utils::remove_dir("component", &prefix.abs_path(path), utils::NotifyHandler::none())),
	...
	fn copy_file(prefix: &InstallPrefix, component: &str, path: String, src: &Path) -> Result<Self> {
		let abs_path = prefix.abs_path(&path);
		if utils::path_exists(&abs_path) {
			Err(Error::ComponentConflict { name: component.to_owned(), path: path.clone() })
		} else {
...
impl<'a> Transaction<'a> {
	pub fn new(prefix: InstallPrefix, temp_cfg: &'a temp::Cfg, notify_handler: NotifyHandler<'a>) -> Self {
		Transaction {
			prefix: prefix,
			changes: Vec::new(),
			temp_cfg: temp_cfg,
	...
	pub fn write_file(&mut self, component: &str, path: String, content: String) -> Result<()> {
		...
		try!(write!(file, "{}", content)
			.map_err(|e| utils::Error::WritingFile { name: "component", path: self.prefix.abs_path(&path), error: e}));
		Ok(())
	}
	...
	fn drop(&mut self) {
		if !self.committed {
			...
			for item in self.changes.iter().rev() {
				ok_ntfy!(self.notify_handler, Notification::NonFatalError, item.roll_back(&self.prefix));
			}
		}
	}
}
