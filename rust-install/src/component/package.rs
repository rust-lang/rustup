    ...
    if is_dir {
        ...
        for entry in WalkDir::new(dest_path) {
            ...
            if meta.is_dir() {
                ...
                perm.set_mode(0o755);
                try!(fs::set_permissions(entry.path(), perm).map_err(|e| {
                    Error::SetPermissions(e)
                }));
            } else {
                let mut perm = meta.permissions();
                perm.set_mode(0o644);
        ...
        let mut perm = try!(fs::metadata(dest_path).map_err(|e| {
            Error::SetPermissions(e)
        })).permissions();
        perm.set_mode(0o755);
        try!(fs::set_permissions(dest_path, perm).map_err(|e| {
            Error::SetPermissions(e)
        }));
    } else {
        let mut perm = try!(fs::metadata(dest_path).map_err(|e| {
