...
impl Manifestation {
    ...
    pub fn update(&self,
        ...
        let dist_manifest = try!(Manifest::parse(&*try!(utils::read_file("dist manifest",
                                                                         &dist_path))));
        let rel_packages_path = prefix.rel_manifest_file(PACKAGES_MANIFEST);
        let packages_path = prefix.abs_path(&rel_packages_path);
        let packages_manifest = try!(Manifest::parse(&*try!(utils::read_file("packages manifes\
                                                                              t",
                                                                             &packages_path))));
        ...
        let new_manifest = try!(dist_manifest.for_root(&old_root,
                                                       &old_target,
                                                       |e| new_extensions.contains(e)));
