...
impl<'a> Notification<'a> {
    pub fn level(&self) -> NotificationLevel {
        ...
        match *self {
            ...
            FileDeletion(_, ref result) | DirectoryDeletion(_, ref result) => if result.is_ok() {
...
impl<'a> Display for Notification<'a> {
    fn fmt(&self, f: &mut fmt::Formatter) -> ::std::result::Result<(), fmt::Error> {
        ...
        match *self {
            ...
            FileDeletion(path, ref result) => if result.is_ok() {
                write!(f, "deleted temp file: {}", path.display())
            } else {
                write!(f, "could not delete temp file: {}", path.display())
            },
            DirectoryDeletion(path, ref result) => if result.is_ok() {
