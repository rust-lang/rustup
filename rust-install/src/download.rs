	...
	pub fn get(&self, url: &str) -> Result<temp::File<'a>> {
		if let Some(key) = self.gpg_key {
			...
			
			{
				...
				try!(utils::download_file(target_url, &target_file, None, ntfy!(&self.notify_handler)));
				...
				try!(utils::cmd_status("gpg", Command::new(gpg)
			...
			let target_url = try!(utils::parse_url(url));
			let target_file = try!(self.temp_cfg.new_file());
			try!(utils::download_file(target_url, &target_file, Some(&mut hasher), ntfy!(&self.notify_handler)));
			...
			if hash != actual_hash {
				// Incorrect hash
				return Err(Error::ChecksumFailed { url: url.to_owned(), expected: hash, calculated: actual_hash });
			} else {
