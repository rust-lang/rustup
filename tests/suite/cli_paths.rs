...
"#;
    ...
    async fn install_creates_necessary_scripts() {
        ...
        // Exercise the default Cargo home; newly generated paths are absolute.
    ...
    async fn category_mode_uses_rustup_homes_for_path_setup() {
        ...
        let data_home = cx.config.homedir.join(".local/share/rustup");
        ...
        assert!(!config_home.join("env").exists());
        assert!(!cx.config.cargodir.join("env").exists());
        let env_file = data_home.join("env");
        ...
        let expected_profile = FAKE_RC.to_owned() + &source(data_home.display(), POSIX_SH);
...
"#]])
    ...
    async fn default_cargo_home_recognizes_legacy_sources_and_cleans_up() {
        ...
        cmd.env_remove("CARGO_HOME");
        assert!(cmd.output().unwrap().status.success());
        // Keep the existing abbreviated command without adding a duplicate.
        assert_eq!(fs::read_to_string(&profile).unwrap(), legacy);

        // Cleanup must remove both abbreviated and absolute commands if both exist.
...
mod windows {
    ...
    async fn install_uninstall_affect_path_with_non_unicode() {
        ...
        USER_PATH
            .set(Some(&reg_value), test_id, CURRENT_USER)
            .unwrap();
        ...
        cx.config.expect(&INIT_NONE).await.is_ok();
        assert_eq!(get_path(test_id).unwrap().unwrap(), expected);
