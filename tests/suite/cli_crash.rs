...
use rustup::dist::manifestation::CHECKPOINT_UPDATE_BEFORE_METADATA;
use rustup::install::{
...
async fn assert_unpublished_install_can_be_retried(checkpoint: &str) {
    ...
    let status = cx.kill_at(checkpoint, ["rustup", "toolchain", "install", "nightly"]);
...
async fn interrupted_install_can_be_retried() {
    assert_unpublished_install_can_be_retried(CHECKPOINT_UPDATE_BEFORE_METADATA).await;
}
...
async fn interrupted_installs_do_not_accumulate_stages() {
    ...
    for _ in 0..2 {
        let status = cx.kill_at(
            CHECKPOINT_INSTALL_BEFORE_PUBLISH,
...
async fn interrupted_install_after_publication_is_complete() {
    let cx = CliTestContext::new(Scenario::SimpleV2).await;
    let status = cx.kill_at(
        CHECKPOINT_INSTALL_AFTER_PUBLISH,
        ["rustup", "toolchain", "install", "nightly"],
    );
    assert!(!status.success());
    assert!(nightly_path(&cx).is_dir());
