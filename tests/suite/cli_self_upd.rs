...
async fn update_exact() {
    ...
    cx.config
        ...
        .await
        .extend_redactions([("[VERSION]", version), ("[TEST_VERSION]", TEST_VERSION)])
        .with_stdout(snapbox::str![[r#"
...
async fn update_no_change() {
    ...
    cx.config
        .expect(["rustup", "self", "update"])
        .await
        .extend_redactions([("[VERSION]", version)])
        .with_stdout(snapbox::str![[r#"
  rustup unchanged - [VERSION]
...
async fn update_does_not_overwrite_rustfmt() {
    ...
    cx.config
        ...
        .await
        .with_stderr(snapbox::str![[r#"
info: checking for self-update (current version: 1.28.2)
