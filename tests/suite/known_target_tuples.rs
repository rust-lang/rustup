...
fn gen_known_target_tuples() {
    ...
    for (arch, os, env) in Platform::ALL
        .iter()
        .map(|p| parse_target_tuple(p.target_triple))
    {
        archs.insert(arch);
        oses.insert(os);
        if !env.is_empty() {
...
/// Parses the given target tuples into 3 parts (target architecture, OS and environment).
///
/// # Discussion
///
/// The current model of target tuples in Rustup requires some non-code knowledge to correctly generate the list.
/// For example, the parsing results of two 2-dash target tuples can be different:
///
/// ```jsonc
...
/// { arch: x, os: y-z, env: w }
/// ```
fn parse_target_tuple(target_tuple: &str) -> (&str, &str, &str) {
    match target_tuple.split('-').collect::<Vec<_>>()[..] {
        [arch, os] => (arch, os, ""),
        [arch, os @ ("none" | "linux"), env] => (arch, os, env),
        [arch, _, _] => (arch, &target_tuple[(arch.len() + 1)..], ""),
        [arch, _, _, env] => (
            arch,
            &target_tuple[(arch.len() + 1)..(target_tuple.len() - env.len() - 1)],
            env,
        ),
        _ => panic!(
            "Internal error while parsing target tuple `{target_tuple}`, please file an issue at https://github.com/rust-lang/rustup/issues"
        ),
    }
}
