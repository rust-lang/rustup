...
fn rustup_stable() {
    ...
    cx.config
        .with_scenario(Scenario::ArchivesV2_2015_01_01, &|config| {
...
fn recursive_cargo() {
    let mut cx = CliTestContext::from(Scenario::None);
    cx.config.with_scenario(Scenario::ArchivesV2, &|config| {
        config.expect_ok(&["rustup", "default", "nightly"]);
...
fn show_multiple_targets() {
    let mut cx = CliTestContext::from(Scenario::None);
    cx.config.with_scenario(Scenario::MultiHost, &|config| {
        config.expect_ok(&[
            "rustup",
            "default",
...
fn check_host_goes_away() {
    let mut cx = CliTestContext::from(Scenario::None);
    cx.config
        .with_scenario(Scenario::HostGoesMissingBefore, &|config| {
            config.expect_ok(&["rustup", "default", "nightly"]);
        });
    cx.config
        .with_scenario(Scenario::HostGoesMissingAfter, &|config| {
            config.expect_err(
