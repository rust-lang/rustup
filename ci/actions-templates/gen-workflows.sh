...
# A new push to a PR cancels the run for its previous push. Every other
# run gets a group of its own so that pushes to main and stable, which
# deploy artifacts, always run to completion.
concurrency:
