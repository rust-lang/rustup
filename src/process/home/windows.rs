...
use super::HomeCategory;

pub(super) fn category_dir(category: HomeCategory, _env: &impl Env) -> io::Result<PathBuf> {
...
mod tests {
    ...
    fn categories_use_known_folders_without_environment() -> io::Result<()> {
        ...
        assert_eq!(category_dir(HomeCategory::Cache, &env)?, local);
        for category in [
            HomeCategory::Config,
            HomeCategory::Data,
            HomeCategory::State,
        ] {
            assert_eq!(category_dir(category, &env)?, roaming);
        }
        Ok(())
    }
