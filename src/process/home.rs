...
mod tests {
    ...
    fn uses_direct_rustup_precedence() -> io::Result<()> {
        ...
        let homes = HomeDirs::from_env(&tp.process)?;
    ...
    fn uses_rustup_bin_home_override() -> io::Result<()> {
        ...
        let tp = TestProcess::new(cwd, &[] as &[&str], vars, "");
        assert_eq!(bin_home(&tp.process)?, Path::new("/work/cargo/bin"));
        Ok(())
    }
    ...
    fn without_absolute_home() -> io::Result<()> {
        ...
        assert!(HomeDirs::from_env(&tp.process).is_err());
        assert!(bin_home(&tp.process).is_err());
        ...
        assert_eq!(bin_home(&tp.process)?, Path::new("relative/.cargo/bin"));
        Ok(())
    }
    ...
    fn uses_cargo_home_or_bin_platform_default() -> io::Result<()> {
        ...
        for (cargo_home, expected_bin_home) in [("", "/home/.local/bin"), ("/cargo", "/cargo/bin")]
        {
            ...
            let tp = TestProcess::with_vars(vars);
            assert_eq!(bin_home(&tp.process)?, Path::new(expected_bin_home));
        }
        Ok(())
    }
