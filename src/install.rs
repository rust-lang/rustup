...
impl InstallMethod<'_, '_> {
    ...
    async fn run(&self, path: &Path, manifest: Option<ManifestWithHash>) -> Result<bool> {
        ...
        match self {
            ...
            // Updates modify the published toolchain in place.
            ...
            // A fresh toolchain is constructed under a private stage and only
            // published once complete, so that an interruption can never
            // leave a partial toolchain behind at a selectable path.
            InstallMethod::Dist(opts) => {
                let staging = StagedToolchain::new(path)?;
                let prefix = InstallPrefix::from(staging.prefix.to_owned());
                // A stray alias-scoped update hash (e.g. left behind by an
                // interrupted uninstall) is deliberately ignored rather than
                // removed: the staged install only ever reads the
                // staging-local hash, and the alias-scoped one is rewritten
                // after publication.
                ...
                opts.cfg
                    .process
                    .checkpoint(CHECKPOINT_INSTALL_BEFORE_PUBLISH);
                ...
                opts.cfg
                    .process
                    .checkpoint(CHECKPOINT_INSTALL_AFTER_PUBLISH);
...
/// A fresh toolchain under construction.
///
/// It remains in a private staging directory, invisible to toolchain enumeration
/// until it is published.
struct StagedToolchain {
    root: PathBuf,
    prefix: PathBuf,
    /// The exclusive lock marking the stage as owned by a live process.
    ///
    /// This is `None` only while dropping.
    lock: Option<fs::File>,
}
...
impl StagedToolchain {
    fn new(destination: &Path) -> Result<Self> {
        ...
        let name = destination
            .file_name()
            .expect("toolchain destination must have a base name");
        ...
        let mut dir_name = OsString::from(STAGING_DIR_PREFIX);
        dir_name.push(name);
        ...
        match lock.try_lock() {
            ...
            Err(fs::TryLockError::WouldBlock) => bail!(
                "toolchain '{}' is already being installed by another rustup process; \
    ...
    /// Returns the update hash tracked next to the stage.
    ///
    /// The hash under `$RUSTUP_HOME/update-hashes` is alias-scoped metadata and
    /// must not be touched before the staged toolchain is published.
