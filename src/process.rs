...
mod tests {
    ...
    fn category_mode_enabled_uses_rustup_bin_home_override() -> io::Result<()> {
        ...
        assert_eq!(
            test_process(cwd, vars).rustup_bin_home()?,
    ...
    #[cfg(unix)]
    #[test]
    fn category_mode_enabled_without_absolute_home() -> io::Result<()> {
        let mut vars = HashMap::new();
        vars.env("RUSTUP_USE_CATEGORY_HOME", "1");
        let process = test_process(Path::new("/work"), vars.clone());

        assert!(process.home_dirs().is_err());
        assert!(process.rustup_bin_home().is_err());
        ...
        assert_eq!(process.rustup_bin_home()?, Path::new("relative/.cargo/bin"));
        Ok(())
    }

    #[cfg(unix)]
    #[test]
    fn category_mode_enabled_uses_cargo_home_or_bin_platform_default() -> io::Result<()> {
        let mut vars = HashMap::new();
        vars.env("RUSTUP_USE_CATEGORY_HOME", "1");
        vars.env("RUSTUP_BIN_HOME", "");
        vars.env("HOME", Path::new("/home"));

        for (cargo_home, bin_home) in [("", "/home/.local/bin"), ("/cargo", "/cargo/bin")] {
