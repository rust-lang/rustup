...
pub(crate) fn uninstall(
    ...
    if process.use_category_home() && !no_modify_path {
        let config_home = &cfg.rustup_config_dir;
        do_remove_from_path(process, config_home)?;
        if config_home != &cargo_home {
            do_remove_from_path(process, &cargo_home)?;
        }
    }
...
fn clean_cargo_home(
        ...
        .chain((category_bin != legacy_bin).then_some(category_bin))
    {
        ...
        if bin_removed && !no_modify_path {
            #[cfg(windows)]
            do_remove_from_path(process, bin)?;
            #[cfg(unix)]
