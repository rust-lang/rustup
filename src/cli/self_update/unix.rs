...
/// Removes shell setup for the given env homes and legacy Cargo setup.
/// Callers select distinct env homes, or none when migrating an installation.
pub(crate) fn remove_from_path(
    ...
    // Group commands by rc file so each file is processed once, in a stable order.
    let mut plan: BTreeMap<PathBuf, Vec<String>> = BTreeMap::new();

    // Plan env script source removal. Env homes never determine bin directories.
    if !env_homes.is_empty() {
        for sh in shell::get_available_shells(process) {
            let mut commands = Vec::new();
            for env_home in env_homes {
                commands.extend([
                    sh.source_string(&env_home.display()),
                    sh.legacy_source_string(env_home, home_dir.as_deref())?,
                ]);
            }
            // Check more files for cleanup than normally are updated.
            for rc_file in sh.rc_candidates(process) {
                plan.entry(rc_file)
                    .or_default()
    ...
    // Cargo home determines the pre-1.23 PATH export and the non-POSIX
    // `source` command accidentally generated in 1.23 (see #2623).
    // Historical POSIX setup abbreviated only the default Cargo home.
    ...
    let commands = [
        format!("export PATH=\"{cargo_home}/bin:$PATH\""),
        format!("source \"{cargo_home}/env\""),
    ];
    for rc_file in shell::legacy_paths(process, home_dir.as_deref()) {
        plan.entry(rc_file)
            .or_default()
            .extend_from_slice(&commands);
    }
...
/// Reads each existing rc file once and writes it back only if changed.
fn apply_removal_plan(plan: BTreeMap<PathBuf, Vec<String>>) -> anyhow::Result<()> {
    for (rc_file, commands) in plan {
        if !rc_file.is_file() {
            continue;
        }
        let mut contents = utils::read_file("rcfile", &rc_file)?;
        let mut changed = false;
        for mut command in commands {
            // Match complete lines, including the trailing newline.
            command.push('\n');
            // Remove only the first match per command, using the updated contents.
            if let Some(start) = find_exact_line(contents.as_bytes(), command.as_bytes()) {
                contents.replace_range(start..start + command.len(), "");
                changed = true;
            }
        }
        if changed {
            utils::write_file("rcfile", &rc_file, &contents)?;
        }
    }
    Ok(())
}

/// Removes historical Cargo setup without removing current shell source lines.
fn remove_legacy_paths(process: &Process, cargo_home: &Path) -> anyhow::Result<()> {
    remove_from_path(process, &[], cargo_home)
}
...
pub(crate) fn add_to_path(process: &Process) -> anyhow::Result<()> {
    ...
    remove_legacy_paths(process, &cargo_home)
}
