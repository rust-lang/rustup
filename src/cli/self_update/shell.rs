...
pub(crate) fn build_source_env_lines(process: &Process) -> String {
    ...
    for shell in get_available_shells(process) {
        let Ok(src) = shell.source_string(&env_home, home_dir.as_deref()) else {
            continue;
        };
...
pub(crate) trait UnixShell {
    ...
    fn format_path(&self, path: &Path, home_dir: Option<&Path>) -> anyhow::Result<String> {
        format_path_with_home("$HOME", path, home_dir)
    }

    fn source_string(&self, env_home: &Path, home_dir: Option<&Path>) -> anyhow::Result<String> {
        let env_home = self.format_path(env_home, home_dir)?;
...
impl UnixShell for Nu {
    ...
    fn format_path(&self, path: &Path, home_dir: Option<&Path>) -> anyhow::Result<String> {
        format_path_with_home("~", path, home_dir)
    }
}
