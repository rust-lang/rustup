...
// Historical path spelling, used only when recognizing old shell commands.
pub(super) fn legacy_env_home<'a>(
    env_home: &'a Path,
    home_dir: Option<&Path>,
    default_home: &'static str,
) -> anyhow::Result<&'a str> {
    if home_dir.is_some_and(|home| env_home == home.join(".cargo")) {
        Ok(default_home)
    } else {
        env_home.to_str().context("Non-Unicode path!")
    }
}
...
pub(crate) fn build_source_env_lines(process: &Process) -> String {
    ...
    for shell in get_available_shells(process) {
        let Ok(src) = shell.source_string(&env_home) else {
            continue;
        };
...
pub(crate) trait UnixShell {
    ...
    fn source_string(&self, env_home: &Path) -> anyhow::Result<String> {
        let env_home = env_home.to_str().context("Non-Unicode path!")?;
        Ok(format!(r#". "{env_home}/env""#))
    }

    // Keep historical command text independent of the current formatter.
    fn legacy_source_string(
        &self,
        env_home: &Path,
        home_dir: Option<&Path>,
    ) -> anyhow::Result<String> {
        let env_home = legacy_env_home(env_home, home_dir, "$HOME/.cargo")?;
        Ok(format!(r#". "{env_home}/env""#))
    }

    fn write_script(&self, script: &ShellScript, process: &Process) -> anyhow::Result<()> {
        let env_home = process.cargo_home()?;
        let bin_home = env_home.join("bin");
...
impl UnixShell for Fish {
    ...
    fn source_string(&self, env_home: &Path) -> anyhow::Result<String> {
        let env_home = env_home.to_str().context("Non-Unicode path!")?;
        Ok(format!(r#"source "{env_home}/env.fish""#))
    }
...
impl UnixShell for Nu {
    ...
    fn source_string(&self, env_home: &Path) -> anyhow::Result<String> {
        let env_home = env_home.to_str().context("Non-Unicode path!")?;
        Ok(format!(r#"source "{env_home}/env.nu""#))
    }
    ...
    fn legacy_source_string(
        ...
        let env_home = legacy_env_home(env_home, home_dir, "~/.cargo")?;
        Ok(format!(r#"source "{env_home}/env.nu""#))
    }
}
...
impl UnixShell for Tcsh {
    ...
    fn source_string(&self, env_home: &Path) -> anyhow::Result<String> {
        let env_home = env_home.to_str().context("Non-Unicode path!")?;
        Ok(format!(r#"source "{env_home}/env.tcsh""#))
    }
...
impl UnixShell for Xonsh {
    ...
    fn source_string(&self, env_home: &Path) -> anyhow::Result<String> {
        let env_home = env_home.to_str().context("Non-Unicode path!")?;
        Ok(format!(r#"source "{env_home}/env.xsh""#))
    }
...
#[cfg(test)]
mod tests {
    use super::{Fish, Nu, Path, Pwsh, Tcsh, UnixShell, Xonsh};
    ...
    fn source_strings_keep_current_and_legacy_formats() {
        ...
        for (shell, current, legacy) in cases {
            assert_eq!(
                shell.source_string(Path::new("/home/user/.cargo")).unwrap(),
                current,
                "{}",
                shell.name()
            );
