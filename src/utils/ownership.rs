//! This is temporary.
//! It will be replaced with `cargo_util` once it is published.
...
pub(crate) fn validate_ownership(path: &Path, safe_directories: &Vec<PathBuf>) -> Result<()> {
    for safe_dir in safe_directories {
        if safe_dir == std::ffi::OsStr::new("*") {
            return Ok(());
        }
...
mod unix {
    ...
    pub(super) fn _validate_ownership(path: &Path) -> Result<()> {
        use std::env;
        ...
        let simulate = matches!(env::var_os("__SAFE_DIRECTORIES_TEST"), Some(p) if path == p);
...
mod windows {
    ...
    pub(super) unsafe fn _validate_ownership(path: &Path) -> Result<()> {
        ...
        let simulate = match env::var_os("__SAFE_DIRECTORIES_TEST") {
