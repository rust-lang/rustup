...
impl Manifest {
    ...
    pub fn get_package(&self, name: &str) -> Result<&Package> {
        self.packages.get(name).ok_or_else(|| Error::PackageNotFound(name.to_owned()).unchained())
    }
        ...
        for (_, pkg) in &self.packages {
            for (_, tpkg) in &pkg.targets {
                for c in tpkg.components.iter().chain(tpkg.extensions.iter()) {
                    let cpkg = try!(self.get_package(&c.pkg).chain_error(|| Error::MissingPackageForComponent(c.clone())));
                    let _ctpkg = try!(cpkg.get_target(&c.target).chain_error(|| Error::MissingPackageForComponent(c.clone())));
                }
            }
        }
...
impl Package {
    ...
    pub fn get_target(&self, target: &TargetTriple) -> Result<&TargettedPackage> {
        self.targets.get(target).ok_or_else(|| Error::TargetNotFound(target.clone()).unchained())
    }
}
