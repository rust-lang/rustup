...
impl<'a> Notification<'a> {
	pub fn level(&self) -> NotificationLevel {
		...
		match *self {
			...
			Utils(ref n) => n.level(),
			Temp(ref n) => n.level(),
			ToolchainDirectory(_, _) | LookingForToolchain(_) |
			WritingMetadataVersion(_) | ReadMetadataVersion(_) =>
...
impl Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter) -> ::std::result::Result<(), fmt::Error> {
		...
		match *self {
			...
			InvalidInstallerUrl => write!(f, "invalid installer url"),
			...
			AlreadyInstalledHere => write!(f, "already installed here"),
