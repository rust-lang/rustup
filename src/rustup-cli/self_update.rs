...
fn delete_rustup_and_cargo_home() -> Result<()> {
    ...
    use winapi::{DWORD, FILE_FLAG_DELETE_ON_CLOSE, FILE_SHARE_DELETE, FILE_SHARE_READ,
                 GENERIC_READ, INVALID_HANDLE_VALUE, OPEN_EXISTING, SECURITY_ATTRIBUTES};
    use kernel32::{CloseHandle, CreateFileW};
...
fn wait_for_parent() -> Result<()> {
    use kernel32::{CloseHandle, CreateToolhelp32Snapshot, GetCurrentProcessId, OpenProcess,
                   Process32First, Process32Next, WaitForSingleObject};
    use winapi::{PROCESSENTRY32, TH32CS_SNAPPROCESS, WAIT_OBJECT_0, DWORD, INFINITE,
...
fn do_remove_from_path(methods: &[PathUpdateMethod]) -> Result<()> {
    ...
    let ref path_str = utils::cargo_home()?
        .join("bin")
        .to_string_lossy()
        .to_string();
